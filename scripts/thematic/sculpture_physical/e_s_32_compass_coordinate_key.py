...
def attack(ciphertext, **params):
    ...
    for key_name, key_raw in key_seqs.items():
        ...
        key_padded = (key_raw * ((ct_len // len(key_raw)) + 1))[:ct_len]

        for direction, dir_name in [(1, "add"), (-1, "sub")]:
            modified = [(ct_num[i] + direction * key_padded[i]) % MOD for i in range(ct_len)]
            score, best_p = check_period_consistency(modified, periods)

            if score >= 6:
                pt_text = ''.join(chr(ord('A') + v) for v in modified)
                method = (f"direct {key_name} {dir_name} "
                          f"period={best_p} key_vals={key_raw[:10]}")
                all_results.append((float(score), pt_text, method))

    # ── With width-7 columnar transposition ─────────────────────────────
    k3_order = [3, 0, 4, 1, 5, 2, 6]
    ...
    for order in test_orders:
        ...
        for key_name, key_raw in key_seqs.items():
            ...
            for direction, dir_name in [(1, "add"), (-1, "sub")]:
                ...
                n_rows = (ct_len + 6) // 7
                n_long = ct_len % 7 if ct_len % 7 != 0 else 7
                ...
                if score >= 6:
                    pt_text = ''.join(chr(ord('A') + v) for v in untransposed[:ct_len])
                    method = (f"columnar order={order} {key_name} {dir_name} "
                              f"period={best_p} key_vals={key_raw[:10]}")
                    all_results.append((float(score), pt_text, method))
...
def main():
    ...
    direct_results = [r for r in results if r[2].startswith("direct")]
    columnar_results = [r for r in results if r[2].startswith("columnar")]

    best_overall = results[0][0] if results else 0

    print(f"\nDirect key results (score >= 6): {len(direct_results)}")
    for score, pt, method in direct_results[:10]:
        print(f"  {score:.0f}/24 | {method}")

    print(f"\nWith width-7 columnar transposition (score >= 6): {len(columnar_results)}")
    for score, pt, method in columnar_results[:10]:
        print(f"  {score:.0f}/24 | {method}")
    ...
    print(f"SUMMARY")
    print(f"{'=' * 60}")
    print(f"  Best score: {best_overall:.0f}/24")
    print(f"  Results >= 6: {len(results)}")

    if results:
        print(f"\n  Top 10:")
        for i, (score, pt, method) in enumerate(results[:10]):
            print(f"    [{i+1}] {score:.0f}/24 | {method}")
    ...
    with open("results/e_s_32_compass_coordinate.json", "w") as f:
        json.dump({
            ...
            "verdict": verdict,
            "results": [{"score": s, "method": m} for s, _, m in results[:20]],
        }, f, indent=2)
