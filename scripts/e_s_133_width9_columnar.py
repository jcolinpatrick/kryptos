...
Variants: Vigenère, Beaufort, Variant Beaufort
Periods tested: 2-14
"""
import itertools
import json
...
def check_periodic_consistency_model_b(perm, period, variant):
    ...
    for i, src in enumerate(perm):
        if src in CRIB_SET:
            ...
            ct_val = CT_NUM[i]
            if variant == 0:    # Vigenère: key = (CT - PT) % 26
                k = (ct_val - pt_val) % MOD
...
def check_periodic_consistency_model_a(perm, period, variant):
...
def main():
    ...
    print(f"Variants: Vigenère(0), Beaufort(1), Variant Beaufort(2)")
    print(f"Models: A (sub→trans), B (trans→sub)")
    print(f"Total checks: 362,880 × 13 × 3 × 2 = {362880 * 13 * 3 * 2:,}")
    print()
    ...
    # At period p, with k constrained positions, random key consistency
    # is approximately k * (1/26) for pairs sharing a residue class
    print("EXPECTED RANDOM BASELINES (approximate):")
    for p in [2, 3, 5, 7, 9, 11, 13]:
        # Rough estimate: if ~24 positions are constrained and spread across
        # p residue classes, each class has ~24/p positions.
        # Probability of 2 random values matching: 1/26
        # Expected consistent = sum over classes of max-in-class
        # For class of size m, expected max ≈ m/26 * 25 + m * 1/26 ≈ m*(25+1)/26 = m
        # Wait, this is more complex. Just note the empirical distribution.
