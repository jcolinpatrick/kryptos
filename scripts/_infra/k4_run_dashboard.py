...
# internaldispatches workers via agent-sdk, which spawns the
...
_PERSONA_RE = re.compile(
    r"persona[:\s]+(\w[\w\-_]+)|Candidate generator:\s*\[?persona\]?\s*([\w\-_]+)",
    re.IGNORECASE,
)
...
def _parse_log(log_path: Path) -> dict:
    ...
    event_re = re.compile(
        ...
        r"candidate generator|critic|worker|SIGNAL|BREAKTHROUGH|ALERT|Error|Traceback|"
...
def _tag_for_line(line: str) -> str:
    ...
    if re.search(r"\bdiscover\b|discovered=|\bbest score\b", low):
        return "SCORE"
    if re.search(r"▸\s*generate|\bcandidate generator\b|generated \d+|\bpersona\b|\bproposed\b", low):
        return "PROPOSAL"
    return "LOG"
...
def _is_redteam_rejected(theory: dict) -> bool:
    """Return True if this theory was vetoed by the adversarial-review.
