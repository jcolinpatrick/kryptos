...
Usage:
    PYTHONPATH=src python3 scripts/_infra/k4_run_postmortem.py \\
        ...
        --out docs/maturation/round2/K4_RUN_POSTMORTEM.md
...
§6.1.5 budget accounting. Companion to docs/maturation/round2/
K4_RUN_PROTOCOL.md.
"""
...
def _is_redteam_rejected(theory: dict) -> bool:
    """True if the theory was vetoed by the red-team-disprover sibling call.
...
def _classify_proposal(theory: dict, exp: Optional[dict]) -> tuple[str, str]:
    ...
    Stage A (theorist never proposed) is handled separately — it's
...
def render_markdown(
    ...
    # Stage A proposals — what theorist mentioned.
    ...
    lines.append("")
    lines.append("Per `docs/maturation/round2/K4_RUN_PROTOCOL.md` §6.1 spec.")
    lines.append("")
    ...
    lines.append(_fmt_row("**A. Theorist never proposed**",
    ...
    if buckets:
        ...
        if max_stage == "E":
            ...
            lines.append("- **Mode A (theorist concentrated on eliminated "
                         "families or missed open-search region).** "
                         "Next brief: 'improve theorist derivation under "
    ...
    lines.append("")
    lines.append(
        "This run dispatched workers via ``claude-agent-sdk``'s "
    ...
    # §6.1.6 architectural finding (R2-maturation-specific).
    lines.append("## 6.1.6 Architectural finding: DSL path not exercised")
    lines.append("")
    lines.append(
        "**[INTERNAL RESULT]** The Round 2 maturation (R2-1 through R2-5) "
    ...
    lines.append(
        ...
        "objects through ``job_dispatcher.execute()`` — workers run freeform "
    ...
    lines.append(
        ...
        "loop (theorist → critic → red-team → freeform-worker), not of the "
        "Round 2 maturation. The mortality distribution is diagnostic for "
    ...
    lines.append(
        ...
        "not another maturation cycle."
    )
    lines.append("")
    lines.append(
        "**Recommended next brief.** A narrow Round 3 phase: "
        "``Wire controller worker path through job_dispatcher.execute().`` "
        ...
        "controller wiring, not a full maturation cycle."
    )
    lines.append("")
    ...
    lines.append(
        ...
        "first tells us what to change about the theorist / critic loop; the "
