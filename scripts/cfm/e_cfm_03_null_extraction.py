...
def attack(ciphertext: str, **params) -> list[tuple[float, str, str]]:
    ...
    for n in range(2, 21):
        for offset in range(n):
            extracted = ciphertext[offset::n]
            if len(extracted) < 3:
                continue
            cov = word_coverage(extracted, 4)
            if cov > 0.0:
                tag = f"null every-{n}th offset={offset}"
                results.append((float(cov), extracted, tag))

    # Test 2: Sculpture row acrostics
    for width in range(8, 16):
        starts = [i for i in range(0, ct_len, width)]
        text_starts = "".join(ciphertext[i] for i in starts if i < ct_len)
        ends = [min(i + width - 1, ct_len - 1) for i in range(0, ct_len, width)]
        text_ends = "".join(ciphertext[i] for i in ends if i < ct_len)
        for label, text in [("row-starts", text_starts), ("row-ends", text_ends)]:
            cov = word_coverage(text, 3)
            if cov > 0.0:
                tag = f"null acrostic width={width} {label}"
                results.append((float(cov), text, tag))

    # Test 3: Fibonacci positions
    fib = [1, 1]
    while fib[-1] < ct_len:
        fib.append(fib[-1] + fib[-2])
    fib_indices = sorted(set(f for f in fib if f < ct_len))
    fib_text = "".join(ciphertext[i] for i in fib_indices)
    cov = word_coverage(fib_text, 3)
    if cov > 0.0:
        results.append((float(cov), fib_text, "null fibonacci"))

    # Test 3b: Prime positions
    primes = [i for i in range(ct_len) if _is_prime(i)]
    prime_text = "".join(ciphertext[i] for i in primes)
    cov = word_coverage(prime_text, 3)
    if cov > 0.0:
        results.append((float(cov), prime_text, "null primes"))
    ...
    for start in range(ct_len):
        ...
        pos = start
        for _ in range(30):
            next_pos = ALPH_IDX[ciphertext[pos]] % ct_len
            ...
            pos = next_pos
        if len(chain) >= 8:
            text = "".join(ciphertext[i] for i in chain)
            cov = word_coverage(text, 3)
            if cov > 0.0:
                results.append((float(cov), text, f"null chain-from-{start}"))

    # Test 5: Grid diagonal reads
    for width in range(7, 15):
        nrows = (ct_len + width - 1) // width
        padded = ciphertext + "X" * (nrows * width - ct_len)
        diag = "".join(padded[r * width + (r % width)]
                       for r in range(nrows)
                       if r * width + (r % width) < len(padded))
        cov = word_coverage(diag, 3)
        if cov > 0.0:
            results.append((float(cov), diag, f"null grid-diag width={width}"))
        adiag = "".join(padded[r * width + (width - 1 - r % width)]
                        for r in range(nrows)
                        if r * width + (width - 1 - r % width) < len(padded))
        cov = word_coverage(adiag, 3)
        if cov > 0.0:
            results.append((float(cov), adiag, f"null grid-antidiag width={width}"))
...
def main():
    ...
    for n in range(2, 21):
        for offset in range(n):
            ...
            if cov > 0.2 or words:
                ...
                raw_results.append((cov, tag, extracted, words))
    ...
    for width in range(8, 16):
        ...
        for label, text in [("row-starts", text_starts), ("row-ends", text_ends)]:
            ...
            if words:
                ...
                raw_results.append((cov, tag, text, words))
    ...
    raw_results.append((word_coverage(fib_text, 3), "fibonacci", fib_text, words))
    ...
    raw_results.append((word_coverage(prime_text, 3), "primes", prime_text, words))
    ...
    for start in range(CT_LEN):
        ...
        if len(chain) >= 8:
            ...
            if words:
                ...
                raw_results.append((word_coverage(text, 3), f"chain-from-{start}", text, words))
    ...
    if raw_results:
        print(f"Best word coverage: {raw_results[0][0]:.1%} — {raw_results[0][1]}")
        print(f"Extraction: '{raw_results[0][2]}'")
        print(f"Words found: {raw_results[0][3]}")
        print("\nTop 10 extractions by word coverage:")
        for cov, tag, text, words in raw_results[:10]:
            if words:
                print(f"  {cov:.1%} | {tag} | words: {[w for _, w in words]}")
    else:
        print("No extractions with English words found.")

    has_signal = any(cov > 0.3 for cov, _, _, _ in raw_results)
    ...
    if attack_results:
        ...
        for score, pt, method in attack_results[:10]:
            print(f"  {score:.3f}  {method:<40}  {pt[:30]}...")
    else:
        print(f"\n── attack() returned 0 results ─────────────────────────────────")
