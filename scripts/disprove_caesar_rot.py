...
def main() -> None:
    ...
    # ── Step 3: Full table — all 25 shifts ─────────────────────────────────
    ...
    for s in range(1, 26):
        ...
        sb = score_candidate(pt)            # canonical scorer
    ...
    best     = [r for r in results if r["crib_score"] == max_crib]
    bc_hits  = [r for r in results if r["bc_ok"]]
    ene_hits = [r for r in results if r["ene_ok"]]
    bean_any = [r for r in results if r["bean"]["passed"]]
