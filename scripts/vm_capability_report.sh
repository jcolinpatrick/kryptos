...
# Authoritative runtime state for Claude sessions.
# Usage: bash scripts/vm_capability_report.sh [--json]
# Output: results/vm_capability.txt (and optionally results/vm_capability.json)
...
JSON_OUT="$OUT_DIR/vm_capability.json"
JSON_MODE=false
...
echo "── EXECUTION GUIDANCE (for Claude sessions) ─────────────────"
WORKERS=$((VCPUS - 2))
RAM_PER_WORKER=$((RAM_MB / WORKERS))
echo "  Default worker count:  $WORKERS (cpu_count - 2)"
echo "  RAM per worker:        ~${RAM_PER_WORKER}MB (if evenly split)"
echo "  Checkpoint threshold:  Jobs >10 min MUST checkpoint"
echo "  Batch reporting:       Print progress every chunk (workers × 2 items)"
echo ""
echo "  For CPU-bound SA/MC/sweeps:"
echo "    pool = multiprocessing.Pool(max(1, cpu_count() - 2))"
echo "    → $WORKERS parallel workers on this machine"
echo ""
echo "  For I/O-bound work (corpus ingest, web fetch):"
echo "    Use ThreadPoolExecutor or asyncio, not multiprocessing"
echo ""
echo "  Single-threaded execution of parallelizable work"
echo "  requires explicit justification per CLAUDE.md policy."
echo ""
echo "══════════════════════════════════════════════════════════════"
...
data = {
    ...
    'ram_per_worker_mb': $RAM_MB // $WORKERS,
    'python_version': '$(python3 --version 2>&1 | cut -d' ' -f2)',
    'venv_present': $([ -f "$PROJECT_DIR/venv/bin/python3" ] && echo 'True' || echo 'False'),
}
