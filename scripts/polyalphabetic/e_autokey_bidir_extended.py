...
def attack(ciphertext: str, **params) -> list[tuple[float, str, str]]:
    ...
    ct_num_local = [ALPH_IDX[c] for c in ciphertext]
    ct_len_local = len(ciphertext)
    crib_num_local = {pos: ALPH_IDX[ch] for pos, ch in CRIB_DICT.items()
                      if pos < ct_len_local}
    ...
    for vkey in ['vig', 'beau', 'vbeau']:
        ...
        for p in range(1, ct_len_local):
            pt, contra, detail, n_det = propagate_autokey(p, vkey)
            if contra:
                continue
            # Build plaintext string from determined positions
            ...
            cm = sum(1 for pos, exp in crib_num_local.items()
                     if pos < len(pt) and pt[pos] is not None and pt[pos] == exp)
            method = (f"Autokey {vname} seed_len={p} "
                      f"det={n_det}/{ct_len_local} cribs={cm}/24")
            results.append((float(qg), pt_text, method))
...
def main():
    print("=" * 78)
    print("AUTOKEY VIGENERE — EXTENDED BIDIRECTIONAL (ALL SEED LENGTHS 1-96)")
    print("=" * 78)
    ...
    if attack_results:
        ...
        for score, pt, method in attack_results[:10]:
            print(f"  {score:+.3f}  {method}  {pt[:40]}...")
    else:
        print(f"\n── attack() returned 0 results (all seed lengths contradicted) ──")
