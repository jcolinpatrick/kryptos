...
- **KryptosBot controller (live entry point):** `PYTHONPATH=src python3 -u kryptosbot/run_controller.py`. See `kryptosbot/ORIENT.md` (one-page operator guide) and `kryptosbot/ARCHITECTURE.md` (full architecture). The legacy `kryptosbot/solve.py` and `kryptosbot/campaign_v2.py` entry points were quarantined in framework maturation Phase 1; `campaign_v2.py` survives only as an `ImportError` stub at `kryptosbot/_archive/campaign_v2.py`. `kryptosbot/RUNBOOK.md` is now a redirect stub pointing at `ORIENT.md`.
