...
- **campaigns/** — Multi-layer campaign runners. `two_layer/` does sub+trans sweeps with checkpoint/resume, coverage, parallel dispatch, evaluation. `w_delimiter/` is a second family. `manifest.py` + `historical_eliminations.py` track what's been tested.
- **admissibility/** — Pre-flight feasibility gates: `periodic_admissibility.py`, `procedure_policy.py`, `corpus_policy.py`, `certificate.py` (audit trails).
- **composition/** — Multi-step composition orchestrator: `orchestrator.py`, `scoring_bridge.py`, `constraints.py` (cross-layer), `registry.py`/`ledger.py` (families/results).
- **detectors/** — Joint two-sided detector for E-FRAC-54. Scores PT + implied running-key tape together (`T = L_PT + L_K - Penalty`) to stop mono DOF from absorbing quadgram budget on 97 chars.
- **language/** — Soft grammar prior for K4 candidate phrases (telegraphic/directive registers, anchor-context plausibility around cribs). **Soft prior only** — never promotes to signal; use for ranking short fill candidates.
...
**`copy/` (repo root)** — Curated software-review snapshot of kryptosbot (114 files, flat layout). **Don't edit directly** — changes belong in `kryptosbot/`. Exclude from grep when working on the live controller.
...
- **`docs/superpowers/`** — Historical March 2026 research planning (specs/plans). Demoted 2026-04-09; many stego plans rest on the retired palette construct. **Not current doctrine** — see the directory's own `README.md` banner before citing anything under it.
...
- **KryptosBot controller (live entry point):** `PYTHONPATH=src python3 -u kryptosbot/run_controller.py`. See `kryptosbot/ORIENT.md` (one-page guide) + `kryptosbot/ARCHITECTURE.md` (full). Legacy `solve.py` and `campaign_v2.py` were quarantined in maturation Phase 1; `campaign_v2.py` survives only as an `ImportError` stub at `kryptosbot/_archive/campaign_v2.py`. `kryptosbot/RUNBOOK.md` is a redirect stub → `ORIENT.md`.
- **Post-R3 control flow (2026-04-21):** Controller worker path dispatches through `job_dispatcher.execute()`; Category-A workers no longer call Claude directly; kernel overrule preserved across DSL handoff. R3-0.5 extended DSL to 9 kinds (added procedural/grille/polybius translators). **`docs/maturation/round3/K4_RUN_PROTOCOL_R3.md` supersedes `round2/K4_RUN_PROTOCOL.md`**; don't follow R2.
