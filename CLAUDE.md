...
Agent SDK research campaign runner. Separate from the core `src/kryptos/` package. One entry point: `python3 <internal>/solve.py`. Key modules: `strategies.py` (22 strategies in 3 modes), `agent_runner.py` (session loop + token tracking), `sdk_wrapper.py` (SDK safety wrapper), `compute.py` (local multiprocessing), `database.py` (SQLite). Requires `agent-sdk` and `python-dotenv` (in venv). Results go to `results/` (gitignored).
