...
*Last updated: 2026-05-03. CLAUDE.md = **operational doctrine only**; live research state in MEMORY.md, structured claims in `docs/claims_registry.json`, audits in `docs/methodological_audits.md`, entry index in `docs/README_current_state.md`. Conflict rule: verify freshness via `git log -1 --format=%cd CLAUDE.md MEMORY.md`; if they disagree on research state, trust MEMORY.md. Operational doctrine in CLAUDE.md is always authoritative regardless of date.*
