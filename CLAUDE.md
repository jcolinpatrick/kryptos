...
Builds the `kryptosbot.com` static site. Output goes to `site/` (gitignored). Key modules: `data_loader.py` (loads experiment data from DBs/artifacts), `categorizer.py` (classifies experiments by method), `search_index.py` (generates client-side search index), `overrides.toml` (per-experiment display overrides). Commands in the "Development Setup" section above.

### API backend (`api/`)

FastAPI backend for kryptosbot.com. Theory classifier endpoint (Claude-powered), submission queue (SQLite), CORS, rate limiting. Mounts `site/` as static files. Commands in the "Development Setup" section above.
