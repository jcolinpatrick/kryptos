...
(`copy/`, a flattened kryptosbot review snapshot, was deleted 2026-06-11 — it contained agent definitions, which stay private.)
...
- **Two `.env` files — don't mix them up**: `.env` (root) = `ANTHROPIC_API_KEY` + `KBOT_CLASSIFY_API_KEY` + `NTFY_TOPIC`. `kryptosbot/.env` = Agent SDK API key (`ANTHROPIC_API_KEY` for the SDK; no template file is checked in — copy the existing `.env` from a working machine or set the var directly). Loading the wrong one gives silent auth failures.
...
- **Pool-worker per-task timeout enforced (since 2026-05-07)**: `job_dispatcher` dispatches workers via `apply_async` + per-future timeout (default 60s), NOT bare `imap_unordered`. A hung worker yields a synthetic `{"error": "per_task_timeout"}` result instead of blocking the cycle. The prior `imap_unordered` path silently deadlocked the entire controller for hours on a single hung worker (Quagmire III BEARING, cycle 245). If you see `per_task_timeout` in results, treat as inconclusive (worker exceeded budget), not as a disproof. Don't reintroduce a no-timeout pool. See `project_pool_worker_timeout_fix_2026_05_07.md`.
- **Transparency doctrine (2026-06-11) — repo is a public gift, push directly**: the repo INCLUDING `kryptosbot/` and `docs/maturation/` is pushed straight to GitHub (`git push origin main`); all git actions including push are authorized. Exactly four classes stay private, enforced by `.gitignore` + `.githooks/pre-push` (`core.hooksPath=.githooks`): (1) `.claude/` and any `SKILL.md` — the precise construction of agents/skills (that a team-of-rivals exists is fine to be deducible from code); (2) `.env` secrets; (3) `reference/`, `archive/`, `analysis_runs/` — local-only source material, bulk archives, photo corpora (shared photos live on kryptosbot.com only); (4) truly personal files (correspondence) are DELETED outright, not just ignored. `ops/publish/publish_to_github.sh` is a retired stub; the content scanner (`ops/publish/scan_content.sh`) still runs from the hook. Never propose removing the hook or publishing the private classes.
