...
```
Every prior experiment (860+, 669B+ configs) assumed positional correspondence on 97 chars and FAILED. The **singular mission** is to derive the full encryption method and solve K4.
...
**Cardan grille (reframed as selection mask):** The grille may SELECT 73 real chars from 97 (original Cardan function — read through holes, ignore blocked positions), rather than reordering all 97. See Claude Code auto-memory `cardan_grille.md` and `73_char_hypothesis.md`.
...
**Null palette discovery (2026-03-15):** [INTERNAL RESULT] The 24 null characters use ONLY the 7-letter palette `{B,G,I,K,O,W,Z}` (p=0.000024). This palette is generated by KRYPTOS×SEVEN dual-keyword Polybius on a 5-wide KA grid (6/6 rows match, p≈1/1672). Beaufort A=0 supported by 7/8 palette enrichment at BCL keystream positions (p=0.000627). Null positions obey a `(pos%7, pos%5)` classification table with 35/35 perfect accuracy — but this is post-hoc with zero degrees of freedom, so treat as [HYPOTHESIS] not [DERIVED FACT]. See MEMORY.md for details.
