#!/usr/bin/env bash
#
# health-check.sh — Lightweight morning health check for internal.com
#
...
# 1. API service
API_STATUS=$(systemctl is-active internal-api 2>/dev/null || echo "inactive")
...
DATE=$(date '+%Y-%m-%d')
if [[ ${#ISSUES[@]} -eq 0 ]]; then
    MSG="internal.com — All OK. API: active, HTTP: ${HTTP_CODE}, Disk: ${DISK_FREE} free"
    echo "[$(ts)] OK: ${MSG}" >> "$LOG_FILE"
    # Only notify on issues, not on success (reduce noise)
else
    MSG="internal.com — ${#ISSUES[@]} issue(s): $(IFS='; '; echo "${ISSUES[*]}")"
    echo "[$(ts)] WARN: ${MSG}" >> "$LOG_FILE"
    notify "internal.com — Health Check" "$MSG" "high" "warning"
fi
