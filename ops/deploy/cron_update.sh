...
if [[ ${EUID:-$(id -u)} -eq 0 ]]; then
    if [[ -n "${SUDO_USER:-}" && "${SUDO_USER}" != "root" ]]; then
        echo "[INFO] Re-running cron_update.sh as ${SUDO_USER} to avoid root-owned output."
        exec sudo -u "$SUDO_USER" -H bash "$SCRIPT_PATH" "$@"
    elif [[ "${CRON_UPDATE_ALLOW_ROOT:-0}" != "1" ]]; then
        echo "Refusing to run cron_update.sh as root."
        echo "Run it as the repo user, or set CRON_UPDATE_ALLOW_ROOT=1 to override."
        exit 1
    fi
fi
...
generate_traffic_stats() {
    log_section "Step 5: Generating GoAccess traffic report"

    STATS_DIR="${REPO_DIR}/site/stats"
    mkdir -p "$STATS_DIR"

    if command -v goaccess &>/dev/null && [[ -f /var/log/nginx/access.log ]]; then
        # Include rotated logs if available for fuller history
        LOG_FILES=(/var/log/nginx/access.log)
        for rotated in /var/log/nginx/access.log.1; do
            [[ -f "$rotated" ]] && LOG_FILES+=("$rotated")
        done

        # Privacy: exclude own/LAN traffic entirely, and scrub referrer strings
        # that --anonymize-ip does NOT touch (referrers can carry the full home
        # IP, its reverse-DNS hostname, and LAN addresses). Flag spelling
        # verified against this machine's goaccess: --ignore-referrer (two r's).
        if goaccess "${LOG_FILES[@]}" \
            --log-format=COMBINED \
            --output="$STATS_DIR/index.html" \
            --html-report-title="kryptosbot.com" \
            --anonymize-ip \
            --ignore-crawlers \
            --exclude-ip=127.0.0.1 \
            --exclude-ip=::1 \
            --exclude-ip=100.18.4.0-100.18.4.255 \
            --exclude-ip=192.168.0.0-192.168.255.255 \
            --ignore-referrer='*100.18.4.*' \
            --ignore-referrer='*192.168.*' \
            --ignore-referrer='*.fios.verizon.net*' \
            --ignore-referrer='*kryptosbot.com*' \
            2>/dev/null; then
            log "GoAccess report generated (${#LOG_FILES[@]} log file(s))"
        else
            log "WARN: GoAccess report generation failed (non-fatal)"
        fi
    else
        log "WARN: GoAccess not installed or no access log — skipping stats"
    fi
}
...
compute_inputs_checksum() {
    ...
    # We hash mtimes (not contents) for speed — any touch triggers rebuild.
    {
        ...
        find ops/site_builder/ -type f -exec md5sum {} + 2>/dev/null | sort
...
if PYTHONPATH=src venv/bin/python3 ops/site_builder/build.py; then
...
# Verify db/ symlink (classify endpoint depends on it)
if [[ ! -d "db" ]]; then
    log "SANITY FAIL: db/ symlink missing or broken — classify endpoint will fail"
    # Attempt auto-repair if target exists
    if [[ -d "/data/db" ]]; then
        ln -s /data/db db && log "REPAIRED: db/ symlink recreated"
    else
        SANITY_OK=false
    fi
fi
