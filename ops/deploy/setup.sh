...
SELFHEAL_SERVICE="$REPO_DIR/deploy/kryptosbot-selfheal.service"
SELFHEAL_TIMER="$REPO_DIR/deploy/kryptosbot-selfheal.timer"

echo "=== 1/7  Installing system packages ==="
sudo apt-get update -qq
sudo apt-get install -y -qq nginx certbot python3-certbot-nginx goaccess
...
sudo ln -sf "$SELFHEAL_SERVICE" /etc/systemd/system/kryptosbot-selfheal.service
sudo ln -sf "$SELFHEAL_TIMER" /etc/systemd/system/kryptosbot-selfheal.timer
sudo systemctl daemon-reload
sudo systemctl enable kryptosbot-api
sudo systemctl restart kryptosbot-api
sudo systemctl enable --now kryptosbot-selfheal.timer

echo "=== 7/8  Setting up GoAccess stats ==="
mkdir -p "$REPO_DIR/site/stats"
if ! [[ -f /etc/nginx/.htpasswd_stats ]]; then
    echo "Stats htpasswd file not found — will create during manual steps."
else
    echo "Stats htpasswd already exists."
fi
...
echo ""
echo "  3. Create stats password (for /stats/ dashboard):"
echo "       sudo sh -c \"echo 'admin:\$(openssl passwd -apr1)' > /etc/nginx/.htpasswd_stats\""
echo ""
echo "  4. Verify:"
