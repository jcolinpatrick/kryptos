...
it only heals the public serving stack (`nginx` + `kryptosbot-api`) and only
...
STATE_PATH = Path("/var/lib/kryptosbot/self_heal_state.json")
...
def execute_action(action: str, env: dict[str, str], state: State, obs: Observation) -> None:
    ...
    if action == "restart_api":
        log(f"SELF-HEAL: restarting kryptosbot-api ({detail})")
        run_systemctl("restart", "kryptosbot-api.service")
        notify(env, "kryptosbot self-heal", f"Restarted kryptosbot-api. {detail}")
    elif action == "restart_public_stack":
        log(f"SELF-HEAL: restarting nginx + kryptosbot-api ({detail})")
        run_systemctl("restart", "nginx.service")
        run_systemctl("restart", "kryptosbot-api.service")
        notify(env, "kryptosbot self-heal", f"Restarted nginx + kryptosbot-api. {detail}")
    elif action == "reboot_host":
        log(f"SELF-HEAL: rebooting host ({detail})")
        notify(
            env,
            "kryptosbot emergency reboot",
