...
tar xzf /path/to/internal.tar.gz

# Install the Agent SDK in a venv (recommended)
python3 -m venv ~/kryptos/<internal>/.venv
source ~/kryptos/<internal>/.venv/bin/activate
...
### Open the internalfolder in VS Code

File → Open Folder → `~/kryptos/<internal>/`
...
choose `~/kryptos/<internal>/.venv/bin/python3`
...
python3 run_internal.py --strategies

# 2. Test local compute engine (no API key needed)
python3 -c "
from internal.compute import run_statistical_profile
...
```bash
python3 run_internal.py --bootstrap --verbose
```
...
```bash
python3 run_internal.py --report
```
...
```bash
python3 run_internal.py --disproofs --verbose
```
...
```bash
python3 run_internal.py --workers 4 --priority 1 --verbose
```
...
cd ~/kryptos/internal&& source .venv/bin/activate
export $(grep -v '^#' .env | xargs)
python3 run_lean.py --local --workers 28
...
cd ~/kryptos/internal&& source .venv/bin/activate
export $(grep -v '^#' .env | xargs)
python3 run_internal.py --report     # see what completed
...
2. `python3 run_internal.py --bootstrap` to re-import
...
add functions to `<internal>/compute.py` following the existing pattern.
...
```bash
python3 run_internal.py --report    # see what finished
python3 run_lean.py --local --workers 28  # continue
```
...
```bash
# Edit <internal>/database.py, change:
...
python3 run_internal.py --bootstrap                   # Import knowledge (~$0.50)
python3 run_internal.py --disproofs                   # Disproof sweep
python3 run_internal.py --single <strategy>           # One strategy
python3 run_internal.py --workers 4                   # Small parallel campaign
...
3.  run_internal.py --bootstrap           Import framework knowledge (~$0.50)
4.  run_internal.py --report              Verify import
5.  run_lean.py --agent                     Agent analysis (~$1-2)
6.  Run agent-written scripts locally       Free
7.  run_lean.py --agent                     Feed back results (~$1-2)
8.  Repeat 6-7 daily
9.  run_internal.py --disproofs           Periodic disproof sweep
```
