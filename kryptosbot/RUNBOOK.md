...
└── <internal>/                         ← campaign runner directory
    ├── solve.py                        ← THE entry point (all commands)
    ├── monitor.py                      ← live dashboard
    ├── RUNBOOK.md                      ← this file
    └── <internal>/                     ← Python package (library modules)
...
# Either export directly or put in <internal>:
echo "ANTHROPIC_API_KEY=sk-ant-api03-your-key" > <internal>
```
...
python3 -c "from internal.internal.strategies import STRATEGIES; print(f'{len(STRATEGIES)} strategies OK')"

# 3. Test Agent SDK (uses a few hundred tokens)
PYTHONPATH=src python3 <internal>/solve.py preflight

# 4. List all strategies
PYTHONPATH=src python3 <internal>/solve.py list
...
python3 <internal>/solve.py

# Local compute only (free, no API tokens)
python3 <internal>/solve.py compute

# Run specific strategies by name
python3 <internal>/solve.py run grille_geometry constraint_solver wildcard

# Reasoning-only agents (bespoke thinking, no code execution)
python3 <internal>/solve.py reason

# Single agent, quick test
python3 <internal>/solve.py run wildcard --max-turns 5

# Full options
python3 <internal>/solve.py --agents 6 --max-turns 25 --budget 50.0

# Utility commands
python3 <internal>/solve.py list          # Show all strategies
python3 <internal>/solve.py preflight     # SDK/auth health check
python3 <internal>/solve.py report        # Show results summary
```
...
cd ~/kryptos
source venv/bin/activate
python3 <internal>/monitor.py --interval 3
python3 <internal>/monitor.py --db results/results.db  # explicit DB path
```

### Results summary

```bash
PYTHONPATH=src python3 <internal>/solve.py report
```
...
# Run your campaign
PYTHONPATH=src python3 -u <internal>/solve.py --budget 30
...
Run from `~/kryptos/`, not from `~/kryptos/<internal>/`:
```bash
cd ~/kryptos
PYTHONPATH=src python3 <internal>/solve.py preflight
```
...
```bash
PYTHONPATH=src python3 <internal>/solve.py --budget 10.00
```
...
```bash
# ── FREE (local compute) ────────────────────────────────────
PYTHONPATH=src python3 <internal>/solve.py compute

# ── CHEAP ($0.50-5) ─────────────────────────────────────────
PYTHONPATH=src python3 <internal>/solve.py run wildcard --max-turns 5

# ── MODERATE ($5-15) ─────────────────────────────────────────
PYTHONPATH=src python3 <internal>/solve.py reason

# ── EXPENSIVE ($15-60) ──────────────────────────────────────
PYTHONPATH=src python3 <internal>/solve.py --budget 30

# ── STATUS (free) ───────────────────────────────────────────
PYTHONPATH=src python3 <internal>/solve.py list
PYTHONPATH=src python3 <internal>/solve.py preflight
PYTHONPATH=src python3 <internal>/solve.py report
python3 <internal>/monitor.py
```
