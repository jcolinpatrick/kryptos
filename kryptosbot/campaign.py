...
Usage:
    PYTHONPATH=src python3 -u <internal>/campaign.py
    PYTHONPATH=src python3 -u <internal>/campaign.py --budget 250
    PYTHONPATH=src python3 -u <internal>/campaign.py --local-only
    PYTHONPATH=src python3 -u <internal>/campaign.py --dry-run
    PYTHONPATH=src python3 -u <internal>/campaign.py --phase product
    PYTHONPATH=src python3 -u <internal>/campaign.py --reset
"""
...
from internal.hypothesis_tester import (
    HypothesisResult,
    K4_LEN,
    PRIORITY_KEYWORDS,
    run_hillclimb,
...
logger = logging.getLogger("internal.campaign")
...
def _null_mask_sa_worker(args: tuple) -> dict:
    ...
    from <internal> import K4, AZ, KA, CRIBS
...
def run_campaign(
    *,
    ...
    client = None
    if not local_only:
        from internal.api_client import KryptosAPIClient

        api_key = _load_api_key()
        if not api_key:
            print("  No ANTHROPIC_API_KEY found.")
            print("  Set key in environment or <internal> for Opus-guided search.")
...
def _print_campaign_summary(state: CampaignState) -> None:
    ...
    print(f"  Resume with: PYTHONPATH=src python3 -u <internal>/campaign.py")
    print(f"{'='*70}\n")
