...
def render_dashboard(db: ResultsDB) -> str:
    ...
    solved = db.get_by_status(
        __import__("internal.config", fromlist=["HypothesisStatus"]).HypothesisStatus.SOLVED
    )
    if solved:
        lines.append(f"\n{GREEN}{BOLD}{'!' * 72}")
...
def main() -> None:
    ...
    db_path = Path(args.db) if args.db else project_root / "internal_results.db"

    # Also check parent directory (for running from <internal>/ subdir)
    if not db_path.exists():
        parent_db = project_root.parent / "internal_results.db"
        if parent_db.exists():
            db_path = parent_db
    ...
    if not db_path.exists():
        ...
        parallel_db = script_dir.parent / "internal_results.db"
        if parallel_db.exists():
            db_path = parallel_db
