...
logger = logging.getLogger("internal.lean")
...
def run_local_unscramble(args: argparse.Namespace) -> Path:
    ...
    if all_results:
        ...
        # If we found a crib hit, write real_ct.json for run_internal.py
        ...
        if signal_candidates:
            ...
            # Store the plaintext and method so run_internal.py can pick it up.
            real_ct_data = {
            ...
            logger.info("Run: python3 run_internal.py to attack the real CT")
            logger.info("=" * 60)
    else:
