...
Be specific and concise."""
    ...
    def reason_about(
        ...
        *,
        max_tokens: int = 8192,
        system_override: list[dict] | None = None,
    ) -> dict | None:
        """Ask Claude to reason about evidence and return structured JSON.

        Args:
            question: The reasoning prompt (should request JSON output)
            materials: Additional context/evidence to include
            system_override: Override system prompt blocks (for caching)
        ...
        old_system = None
        if system_override:
            old_system = self._system_blocks
            self._system_blocks = system_override
        ...
        try:
            ...
            # Update conversation history
            self._append_to_conversation("user", user_msg[:2000])
            self._append_to_conversation("assistant", text[:2000])
            ...
            # Fallback: try array
            arr = self._parse_json_from_text(text)
            if arr:
                return {"hypotheses": arr}

            logger.warning("reason_about: failed to parse response as JSON")
            return {"raw_text": text[:3000]}
        finally:
            if old_system is not None:
                self._system_blocks = old_system

    def _parse_json_object(self, text: str) -> dict | None:
        """Parse a JSON object from text, handling markdown wrapping."""
        import re
    ...
    def generate_hypotheses_v3(
        ...
        *,
        ...
        """Generate hypotheses using V3 prompts (JSON array output)."""
        old_system = None
        if system_override:
            old_system = self._system_blocks
            self._system_blocks = system_override
        ...
        try:
            ...
            text = self._extract_text(response)
            self._append_to_conversation("user", prompt[:2000])
            self._append_to_conversation("assistant", text[:2000])
            ...
            # Try as object with hypotheses key
            obj = self._parse_json_object(text)
            if obj and isinstance(obj, dict):
                for key in ("hypotheses", "top_3_hypotheses", "archetype_candidates"):
                    if key in obj and isinstance(obj[key], list):
                        return obj[key]

            logger.warning("generate_hypotheses_v3: no valid hypotheses parsed")
            return []
        finally:
            if old_system is not None:
                self._system_blocks = old_system
