...
Usage:
    python run_internal.py                    # Full campaign (all strategies)
    python run_internal.py --disproofs        # Run disproof strategies only
    python run_internal.py --single <name>    # Run one specific strategy
    python run_internal.py --report           # Print current status report
    python run_internal.py --strategies       # List all available strategies
    python run_internal.py --workers 16       # Override worker count
...
from internal.database import ResultsDB
...
def setup_logging(log_dir: Path, verbose: bool = False) -> None:
    ...
    root_logger = logging.getLogger("internal")
...
def print_strategies() -> None:
    ...
    from internal.framework_strategies import FRAMEWORK_STRATEGIES
...
def _run_preflight() -> None:
    ...
    from internal.sdk_wrapper import preflight_check
...
def build_config(args: argparse.Namespace) -> KryptosBotConfig:
    ...
    config = KryptosBotConfig(
        ...
        results_db_path=project_root / "internal_results.db",
...
def main() -> None:
    ...
    if args.report:
        project_root = Path(os.environ.get("KBOT_PROJECT_ROOT", ".")).resolve()
        print_report(project_root / "internal_results.db")
        return
    ...
    from internal.config import K4_CIPHERTEXT, K4_SOURCE
