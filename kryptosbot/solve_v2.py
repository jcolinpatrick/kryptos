...
Usage:
    ...
    python3 <internal>/solve_v2.py

    # Cheap test run
    python3 <internal>/solve_v2.py --budget 2 --rounds 1

    # Deep with Opus + conversation memory
    python3 <internal>/solve_v2.py --model claude-opus-4-6 --budget 30 --rounds 10

    # Disable conversation mode (independent rounds)
    python3 <internal>/solve_v2.py --no-conversation

    # With batch evaluation of top candidates
    python3 <internal>/solve_v2.py --batch-eval --budget 15

    # Free local-only (built-in reading orders + hill-climb)
    python3 <internal>/solve_v2.py --local-only

    # Show cost estimate
    python3 <internal>/solve_v2.py --dry-run
"""
...
logger = logging.getLogger("internal.solve_v2")
...
def run_loop(
    *,
    ...
    if not api_key:
        print("ERROR: No ANTHROPIC_API_KEY found. Set it in environment or <internal>")
        sys.exit(1)
