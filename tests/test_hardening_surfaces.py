...
def _direct_retired_imports() -> list[tuple[str, str, tuple[str, ...]]]:
    ...
    for base in (ROOT / "src", ROOT / "kryptosbot"):
        for path in base.rglob("*.py"):
            tree = ast.parse(path.read_text())
            for node in ast.walk(tree):
                if not isinstance(node, ast.ImportFrom):
                    continue
                if node.module not in {"kryptos.kernel.constants", "kryptosbot.constants"}:
                    continue
...
def test_retired_palette_constants_only_imported_in_quarantined_modules():
    ...
    or kryptosbot.constants.

    Retired constants moved to kryptos.kernel.retired in framework maturation
    ...
    assert not hits, (
        ...
        "(kryptos.kernel.constants / kryptosbot.constants). "
