...
is a regression of the framework maturation Phase 2 quarantine (2026-04-20).
...
_ALLOW_LIST: frozenset[str] = frozenset(
    {
        ...
        # Retired kryptosbot scorer (module docstring: "RETIRED 2026-04-14"):
        "kryptosbot/polybius_scorer.py",
...
def _live_python_files():
    ...
    Skips: the retired namespace itself, the kryptosbot `_archive/` and any
    ...
    for p in (_ROOT / "src").rglob("*.py"):
        ...
        if "retired" in rel:
            continue
        yield p, rel
    for p in (_ROOT / "kryptosbot").rglob("*.py"):
        rel = p.relative_to(_ROOT).as_posix()
        if "_archive" in rel:
            continue
        yield p, rel
