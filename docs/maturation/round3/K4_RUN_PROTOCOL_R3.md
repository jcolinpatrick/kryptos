...
**Supersedes:** `<internal>/K4_RUN_PROTOCOL.md` for all post-R3 operational decisions. The Round 2 version is preserved for historical continuity; do NOT delete or overwrite.
...
- **R2-5 real-API K1 pass** — verified 2026-04-21 in `<internal>/self_test_real_api.py`.
...
- **R3-2 DSL dispatch live** — controller's `_run_worker` routes Category-A (cipher-family) theories via `dispatcher.execute()` with kernel-overrule preserved. Workers on the Category-A path no longer call Claude.
...
1. **Five cipher kinds still lack translators:** `rail_fence, route, myszkowski, quagmire, key_tape`. Candidate generator-proposed theories using any of these get `dsl_untranslatable` rejection at the critic. Operator should expect a non-zero rate of such rejections in the mortality table; they are the framework honestly reporting "this cipher family can't be tested yet," not errors.
...
3. **Agent output reliability** — observed during R3-3 real-candidate generator measurement. The candidate generator's dry-run agent occasionally fails to produce parseable JSON output, causing `_propose_theories` to fall through to `_programmatic_fallback`. The fallback emits "Explore X family" template stubs which then get critic-approved in some Category-B families. Operator must monitor this rate during the K4 run (see §6.1.7 below); sustained fallback rate > 30% is an operator intervention signal.
...
1 candidate generator call +
N parallel calls (red-team, stat-audit, pursuit, synthesis)
```
...
1 candidate generator call shared with Category A +
N parallel calls per Category-B contract
```
...
- **Candidate generator input/output tokens** — rough cycle average 50K–150K tokens (prompt is large; output is per-N-theories).
- **Parallel calls** — red-team ~1 per approved theory, stat-audit ≤1 per signal contract, pursuit ≤1 per sub-signal contract, synthesis 1 per cycle.
...
**Implication:** the budget that previously sized 4 cycles of K4 now sizes roughly 15–25 cycles under typical Category-A:Category-B ratios (estimated 3:1 or better per candidate generator distribution), before the parallel calls dominate.
...
2. Three consecutive cycles with `_programmatic_fallback` firing (fallback rate > 100% for 3 consecutive cycles) — candidate generator agent is broken; investigate before continuing.
...
|---|---|---|
| A. Candidate generator never proposed | families/keywords/recipes absent | — |
...
- Candidate generator calls per cycle (≈ 1)
- Parallel calls per cycle (red-team, stat-audit, pursuit, synthesis)
- Category-B worker subprocess tokens (legacy path)

#### 6.1.6 Architectural finding (R3 closure)

Postmortem must confirm that D column is non-zero at least once per ≥5 dispatched cycles. Absence of any D column entries is an architectural regression — either admissibility isn't firing or all candidate generator-proposed specs are trivially admissible. Both are concerning and warrant investigation.
...
Before invoking `<internal>/solve.py` or `<internal>` for a K4 run, the operator runs:

```bash
# 1. Doctor + full suites
PYTHONPATH=src python3 -m kryptos doctor
PYTHONPATH=src pytest tests/ -q
PYTHONPATH=src pytest <internal>/tests/ -q
...
- The live controller dispatches cipher-family theories through `dispatcher.execute()` (the postmortem §6.1.6 closure).
- Category-B theories route to the legacy path with explicit tags, preserving the candidate generator paradigm while still closing the DSL gap for cipher work.
...
- **Candidate generator cryptanalytic depth.** The DSL's 9-kind coverage is plumbing, not intelligence. Whether the candidate generator proposes truly novel hypotheses inside that space is a prompt-quality concern, not an R3 deliverable.
- **Real-candidate generator spec-production rate above a statistically strong N.** R3-3's measurement observed 100% on N=1 — satisfies the 80% floor but thin evidence. The K4 run's 15-cycle window will produce the first real statistical sample.
...
- [ ] Read the R3 round SUMMARY (`<internal>/SUMMARY.md`).
- [ ] Read `<internal>/K4_RUN_POSTMORTEM.md` §6.1.6 (what R3 was designed to fix).
