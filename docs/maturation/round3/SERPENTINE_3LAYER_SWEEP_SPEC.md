...
**Authored:** 2026-04-22, after Campaign B's null 2-layer sweep and the theorist's three independent serpentine-Vigenère proposals across three personas.
...
Additionally, the theorist independently converged on serpentine-Vigenère three times across three personas during Campaign B (blocked from dispatch by a prompt bug, now fixed). That convergence is mild but real evidence that the family is naturally accessible to a competent theorist reading K4's crib structure. Whether that reflects the mechanism's actual proximity to K4, or just that serpentine is a surface-plausible shape any reasoner would land on, is not resolved by the 2-layer null alone — 3-layer characterization is what would move that question.
