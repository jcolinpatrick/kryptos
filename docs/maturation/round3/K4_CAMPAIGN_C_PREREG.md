...
Did the `_render_oranchak_corpora_for_prompt` block, introduced in the Campaign-A hardening and retained through Campaign B, **actually influence theorist behavior**, or was the observed Oranchak-referencing theory rate (2 in A, 5 in B) driven by something else (e.g., landscape state, persona rotation, Campaign-B's additional serpentine anchor)?
...
- No code changes to either renderer. Both gates live in `_build_theorist_prompt`.
...
- **Total theories proposed.** A = 77, B = 96. C within ±20% of B is consistent with "prompt-content change only"; sharper swings suggest prompt-length effects on theorist economics.
...
- **Serpentine-route-family usage.** Campaign B's serpentine anchor was part of the Oranchak prompt block. C will strip that anchor along with the keyword pools. Expect `route` / `rail_fence` / `myszkowski` dispatch counts to drop. The verification run showed these dispatch cleanly when proposed; C tests whether the theorist still proposes them without the serpentine-Vigenère prompt pairing.
...
- **Prompt-length effects.** The Oranchak block is ~1.2KB (post-B-DSL-expanded expansion). C's theorist runs against a prompt shorter by ~1.2KB. If C's theorist parse rate or tool-use distribution shifts materially, the shift could be attributable to prompt length rather than Oranchak content specifically. This is a known confound; document but don't re-derive conclusions under it.
...
Removing ~1.2KB of content from the theorist prompt *may* affect parsing rates or tool-use economics. If Campaign C shows a material drop in `theorist_parse_successes` (say, below A's 80% / B's ~equivalent) or a jump in tool-use counts, the postmortem must flag this and optionally motivate a Campaign D with a same-length-non-Oranchak filler block.
...
- **Campaign-C addition:** `oranchak_block_present_in_prompt` — boolean, sampled from the first `_build_theorist_prompt` invocation, confirms the toggle took effect.
...
- [x] All 859 kryptosbot tests + 1557 core tests pass
...
- `_build_theorist_prompt` gates each independently via `include_oranchak_corpora` and `include_serpentine_anchor`.
...
**What this means for Campaign C.** The research question ("do the Oranchak keyword pools change theorist behavior?") is measured cleanly with the anchor held constant across A, B, and C. Serpentine-family proposals in C are interpretable against the control (anchor-present) rather than confounded by anchor absence.
