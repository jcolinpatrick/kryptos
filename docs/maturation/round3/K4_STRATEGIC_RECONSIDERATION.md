...
The 3-layer serpentine-adjacent sweep completed 2026-04-22 at wall time 26.8 minutes across **1.573 billion parameter combinations** (certificate: `<internal>/SERPENTINE_3LAYER_SWEEP_CERT.md`).
...
Combined with the 2-layer sweep's prior null (max crib = 9, 8.27M combinations), the serpentine-adjacent additive-inner hypothesis is **durably eliminated across both 2-layer and pruned-3-layer envelopes**. The candidate generator's three-persona convergence on serpentine-Vigenère during Campaign B was a legible hypothesis signal — but across 1.58 billion direct tests, no parameter in the tested envelope solves K4.
