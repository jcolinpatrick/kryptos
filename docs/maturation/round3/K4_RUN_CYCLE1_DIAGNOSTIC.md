...
The candidate generator produced an **18,552-character, well-formed JSON array of 5 cogent K4 hypotheses** — including three Category-A theories with valid, translatable DSL specs (vigenere, grille, atbash+vigenere). The controller silently discarded all of it and fell through to `_programmatic_fallback`, which emitted 5 "Explore X family" template stubs. The fallback stubs then burned one cycle of critic + red-team compute before dying.
...
**Operator instinct was correct.** The D=0 invariant fired at one cycle. The dispatcher wasn't broken; the upstream candidate generator-output path silently failed.

---

## Question 1 — What did the candidate generator actually return on cycle 1?

**The candidate generator ran cleanly and returned a valid, complete response.** No SDK error, no length truncation, no prose collapse.
...
Raw response dumped to `results/k4_run_2026_04_21_r3/candidate generator_cycle1_raw.txt`.
...
The candidate generator used canonical anomaly_ids as prefixes but appended free-form commentary, which the exact-match validator rejected. **This is a secondary concern for a later hygiene pass.** The primary bug is different — see Q5.

---

## Question 2 — Which prompt section did the candidate generator follow?

**The candidate generator followed the DSL_SPEC CONTRACT correctly.** Three Category-A theories carry valid specs; two Category-B theories correctly set `dsl_spec=None`. The prompt integrity test (from R3-3) passes against this output.
...
**Yes.** Source-verified at `<internal>:1593`:
...
## Question 5 — Expected vs actual candidate generator output shape; is there a hidden template collision?

**Yes. The collision is at the SDK-message-to-text serialization boundary, not at the prompt level.**

### The flow

1. Candidate generator agent streams an `AssistantMessage` whose `content` attribute is a list of `ContentBlock` objects (thinking block + text block).
...
```python
fake_msg = f"[TextBlock(citations=None, text={candidate generator_text!r}, type='text')]"
...
The R3-3 measurement's run #1 and run #5 happened to include a `ResultMessage`; runs #2, #3, #4 and this K4 cycle 1 did not. R3-3 documented "non-deterministic fallback firing" as an operator-facing concern but attributed it to candidate generator reliability. The real cause is this serialization bug.
...
**Test to add:** a unit test that feeds a synthetic `AssistantMessage.content = [TextBlock(text='[{"title":...}]')]` through the candidate generator parse path and asserts `report.valid` reflects the embedded theories — not the fallback.
...
The R3-3 phase report flagged fallback-firing as a reliability concern but attributed it to candidate generator output. The real cause was one level deeper — the controller's message-stringification code. Both R3-3 and this run would have been caught by a single test: dispatch a synthetic AssistantMessage with a list of ContentBlocks through the candidate generator parse path.
