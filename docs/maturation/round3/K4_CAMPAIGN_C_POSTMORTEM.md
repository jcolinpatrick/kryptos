...
**Preregistration:** `docs/maturation/round3/K4_CAMPAIGN_C_PREREG.md` (post-split-update, commit `1fbce36`).
...
**Attribution target:** isolated effect of the Oranchak community reference corpora (keyword pools + K4 fills CSV) on theorist behavior, with the AAA-archive serpentine anchor held constant across A / B / C as control surface.
...
- 5 × `quagmire variant='III' unsupported` — theorist proposed classical Quagmire-III label; translator requires `quagmire_iii` (underscore + lowercase). The naming mismatch is a minor prompt footgun that predates Campaign C and affects all post-B runs; worth a one-line fix to the DSL prompt contract.
- 1 × `quagmire_iii requires ct_alphabet_keyword == pt_alphabet_keyword` — theorist proposed different alphabets; the K1/K2 convention gate held.
...
9 distinct anchor families touched. Campaigns A and B each touched 6 core anchors (`ct_perturbation`, `k2_coords`, `compass`, `w_delimiter`, `archive_evidence`, `grille`). Campaign C's expansion into `encoding`, `antipodes`, `k3_continuity`, `geodetic`, `crib_analysis`, and `mirror_ka` suggests theorists broadened the accessible anchor surface when the Oranchak block was removed — consistent with "the block was narrowing proposals toward Oranchak-adjacent work."
...
No landscape-layer leakage, pursuit-lead carry-over, or theorist-memory bleed observed. The toggle took effect cleanly.
...
Directional drop on `beaufort` and `variant_beaufort`; `quagmire` actually **increased** in C. The quagmire increase is striking because the prompt's quagmire guidance (K1/K2 convention enforcement) was inside the corpora block that was removed — yet theorists still proposed quagmire at a higher rate. Two plausible explanations:

1. The K1/K2 convention knowledge is absorbed into the theorist's persona priors (from repeated exposure across Campaigns A-B and verification runs in the ledger landscape), not the prompt block itself. The block's guidance was a reminder, not the source.
...
The archive-derived serpentine-Vigenère anchor was held constant across A / B / C (`include_serpentine_anchor=True` in all three). Campaign C's log contains **57 serpentine/Vigenère-pairing references**, confirming the anchor continued to draw theorist attention under the no-corpora configuration.
...
The Oranchak community corpora block, when present, produced 2-5 Oranchak-referencing theories per 15-cycle campaign. Removing the block drops that count to **0 in 15 cycles**. The community corpora were influencing theorist proposals at the most direct measurable level (explicit path references in generated test specs).
...
- `quagmire` rose (6 in C vs 4 in B). This suggests quagmire-family knowledge is in the theorist priors / post-B DSL disambiguation block, not the Oranchak block.
...
**Null K4 result stands.** Max crib_score 7 in C (A: 8, B: 8). All three campaigns consistent with no K4 signal under any prompt configuration. The attribution result is about *theorist behavior*, not about K4 solvability.
...
- **Oranchak attribution at the proposal level.** The community corpora block measurably influences theorist proposals (A:2, B:5 → C:0). Attribution verdict committed.
...
- **Campaign-A hardening continues to hold.** 0 halts, 0 fallback, 100% theorist parse success across three campaigns.
...
- Preregistration: `docs/maturation/round3/K4_CAMPAIGN_C_PREREG.md` (commit `1fbce36`)
