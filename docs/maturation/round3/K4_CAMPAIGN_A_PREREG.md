# Campaign A — R3 Internal Evaluation (preregistration)

**Commissioned:** 2026-04-22
**Readiness-gate commit:** `3eee572e` (doctor 20/0, tests/ 1532, <internal>/tests/ 805, self-test K1/15 K2/17 K3/9345, R2-5 real-API K1 pass)
...
Does the post-internal candidate generator → DSL → worker loop (Round 3 as of 2026-04-21) produce *informative* elimination/confirmation evidence? Can the internal review system propose novel cipher hypotheses and run them via DSL hooks in a way that meaningfully extends the project's elimination frontier, or is the system's productive output primarily adversarial disproof of its own proposals?
...
```bash
PYTHONPATH=src python3 -u <internal> --cycles 15 --theories 5
```

- `--cycles 15` per R3 §3.3 recommendation.
- `--theories 5` per R3 §3.3.
- No `--dry-run`, no `--skip-critic`.
- Oranchak reference corpora (wordlists + candidate fills) plumbed into candidate generator prompt via `_render_oranchak_corpora_for_prompt` (Campaign-A hardening, 2026-04-22).
- CIA 1996 memo deliberately excluded (Tier-3 hearsay).

## 3. Success criteria (preregistered — ALL must hold for "internal pass")

The adversarial-review flagged the pre-hardening criteria as "satisfiable by R3 wiring firing at least once while the research question goes unanswered." The strengthened criteria below require **representative firing**.
...
**Rationale:** The worked examples lead the candidate generator toward Vigenere/columnar shapes; a matched-null consultation on those does not test the system's breadth. Requiring a `beaufort` / `variant_beaufort` matched-null hit forces the candidate generator to exercise the R2-4 matched-null infrastructure on a family it wasn't handed.
...
- **Max single-stage fraction ≤ 60%**: no single mortality stage (candidate generator-never-proposed / critic-rejected / dsl-untranslatable / red-team-killed / dispatcher-rejected / scoring-outcomes / error) absorbs more than 60% of approved theories.
- **Stage-E (scoring outcomes, dispatched) ≥ 20%**: at least 20% of critic-approved theories reach actual worker scoring.

**Rationale:** The red team's strongest objection was that "non-degenerate" was unfalsifiable. Stage-E ≥ 20% directly tests whether the candidate generator/critic/red-team chain produces theories worth testing, vs. a system that just adversarially disproves everything internally.
...
1. **Fallback streak (≥3 consecutive cycles)** — candidate generator agent broken; halt and investigate before committing more compute.
...
- **Candidate generator cryptanalytic depth at large N.** 15 cycles × 5 theories = 75 proposals is a first statistical sample per R3 §8.2. Depth claims require larger samples.
- **DSL coverage of unsupported kinds.** Candidate generator proposals in `{quagmire, rail_fence, route, myszkowski, key_tape}` will be rejected with `dsl_untranslatable`. This is a known coverage gap, not a campaign failure. The rejection rate on these kinds IS informative.

## 6. Oranchak injection scope

The Oranchak corpora are exposed to the candidate generator via the new `_render_oranchak_corpora_for_prompt` block:
...
The block carries:
- A usage note pointing candidate generators at `vigenere` / `variant_beaufort` (DSL-supported) instead of `quagmire` (unsupported).
...
- Running the architecture eval on a depleted candidate generator surface biases against finding signal.
...
*This preregistration locks the success criteria. Any post-hoc weakening of thresholds invalidates the internal evaluation.*
