...
| **Columnar (w6/8/9) + ANY additive key (running, periodic, or otherwise) — SOURCE-INDEPENDENT** | **Under the full 242-inequality Bean constraint, ZERO of 403,920 orderings × {Vig, Beau, VarBeau} = 1,211,760 (ordering, variant) pairs admit a Bean-consistent keystream. 17,124 orderings pass Bean equality alone but every one violates at least one of the 242 variant-independent inequalities. This is a strict upgrade of E-FRAC-49: that result tested 7 specific reference texts under the OLD 21-inequality Bean and found 0 matches empirically; the upgraded statement is that NO source text of any length or content can produce a solution at widths 6/8/9 because the Bean pre-filter is vacuous before any source is consulted. Independently re-verified 2026-04-09 from a fresh interpreter with from-scratch columnar inverse and Bean check.** | **Requires cribs, CT correctness, and full 242-inequality BEAN_INEQ constant. Certificate: docs/exhaustion_certificate_2026_04_08.md §4-5; campaign: scripts/campaigns/f_final_checklist_c1_c2.py; result: results/f_final_checklist_c1_c2.json** |
| **Columnar (w4) + ANY additive key — SOURCE-INDEPENDENT** | **Under the full 242-inequality Bean constraint, ZERO of 24 orderings × {Vig, Beau, VarBeau} = 72 (ordering, variant) pairs admit a Bean-consistent keystream. w4 was not previously individually documented under the source-independent framing; this closes it alongside w5 (Bean-eliminated), w6/8/9 (certificate §4-5), and w7 (Bean-eliminated). Discovered 2026-04-09 while operationalizing the archive-term "4, 8, 10, 26 = Col" under the new procedure policy (bin D3). Small enumeration: trivial fresh-process re-verification.** | **Requires cribs, CT correctness, and full 242-inequality BEAN_INEQ constant. Campaign: scripts/campaigns/f_archive_col_notation_v1.py (if landed); ad-hoc re-verification also works (72 cases is trivially enumerable).** |
...
| Running Key (K1–K3 as keystream) | ~45,000 | 7/24 | ELIMINATED | **ELIMINATED for known texts + structured transpositions** (E-FRAC-49/50: columnar w6/8/9, affine, cyclic, double columnar × 7 reference texts × 3 variants = 0 matches out of 17B checks). **ELIMINATED for unknown English text + columnar** (E-FRAC-51: 0/16,597 in English range). **ELIMINATED for K1/K2/K3 as key** (E-JTS-12: 0 matches). **ELIMINATED for KA/PAL/ABS alphabets** (E-TABLEAU-21). **STRENGTHENED 2026-04-08 (certificate §4-5):** at columnar widths 6/8/9 the elimination is now SOURCE-INDEPENDENT under the full 242-inequality Bean constraint (ZERO of 1,211,760 (ordering, variant) pairs admit a Bean-consistent keystream); no source text of any length can produce a solution at those widths. **OPEN only for non-columnar transpositions, columnar widths outside {6,8,9}, or non-additive cipher models.** |
