...
3. **Direct positional crib mapping**: the 24 crib letters sit at positions 21–33 and 63–73 of the *carved* CT exactly as they appear on the sculpture. Any mechanism that rearranges or remaps the carved CT *before* the analyzed cipher layers (an outer transposition, a position-dependent selector, or a physical-overlay remap) breaks the Bean inequality derivation and invalidates every Bean-based Tier 1 row below.
4. **The analyzed cipher class uses an additive key model** — i.e., CT[i] = f(PT[i], K[i]) mod 26 for some fixed function f (Vigenère, Beaufort, or Variant Beaufort). If K4 uses a non-additive cipher (lookup table, physical overlay, grid-based system, or any mechanism where Bean's keystream derivation does not apply), ALL Bean-based eliminations are invalidated.

**Scope of "source-independent" wording in the rows below.** Several rows use the phrase *"running-key-source-independent"* (previously written *"SOURCE-INDEPENDENT"* — rewritten 2026-04-09 to close AUDIT-1 in `docs/methodological_audits.md`). That phrase means exactly **"independent of the choice of running-key source text, within the analyzed cipher class"**. It does **not** mean:
- independent of the direct-positional crib mapping (assumption 3 above),
- or independent of whether an outer layer precedes the analyzed layers,
- or universal impossibility across all composed ciphers.
A claim of running-key-source-independence at widths {w} therefore eliminates *"running-key + columnar(w) + Vig/Beau/VarBeau on the carved CT under direct positional crib mapping"*, and nothing broader.
...
| **Columnar (w6/8/9) + additive key (Vig/Beau/VarBeau) — running-key-source-independent within the analyzed class** | **Under the full 242-inequality Bean constraint, ZERO of 403,920 column orderings × {Vig, Beau, VarBeau} = 1,211,760 (ordering, variant) pairs admit a Bean-consistent keystream on the carved 97-char CT under direct positional crib mapping. 17,124 orderings pass Bean equality alone but every one violates at least one of the 242 variant-independent inequalities. Because the Bean pre-filter empties before any running-key scan occurs, the elimination holds for every running-key source text regardless of length or content — within this exact cipher class and under the assumptions stated above. Strict upgrade of E-FRAC-49 (which tested 7 specific texts under the old 21-ineq Bean). Independently re-verified 2026-04-09 from a fresh interpreter with from-scratch columnar inverse and Bean check. **Does NOT eliminate: composed ciphers where an outer layer precedes the columnar step, non-additive keystreams, or cipher classes that break the direct positional crib mapping.** See AUDIT-1 (now closed, 2026-04-09) in `docs/methodological_audits.md` for the scope history.** | **Requires cribs, CT correctness, direct positional crib mapping, and full 242-inequality BEAN_INEQ constant. Certificate: docs/exhaustion_certificate_2026_04_08.md §4-5; campaign: scripts/campaigns/f_final_checklist_c1_c2.py; result: results/f_final_checklist_c1_c2.json** |
| **Columnar (w4) + additive key (Vig/Beau/VarBeau) — running-key-source-independent within the analyzed class** | **Under the full 242-inequality Bean constraint, ZERO of 24 orderings × {Vig, Beau, VarBeau} = 72 (ordering, variant) pairs admit a Bean-consistent keystream on the carved CT under direct positional crib mapping. Closes w4 alongside w5, w6/8/9 (certificate §4-5), and w7 (Bean-eliminated). Discovered 2026-04-09 while operationalizing the archive-term "4, 8, 10, 26 = Col" under the new procedure policy (bin D3). Small enumeration: trivial fresh-process re-verification. **Does NOT eliminate composed ciphers with an outer layer preceding the columnar step, non-additive keystreams, or cipher classes that break the direct positional crib mapping.** | **Requires cribs, CT correctness, direct positional crib mapping, and full 242-inequality BEAN_INEQ constant. Campaign: scripts/campaigns/f_archive_col_notation_v1.py (if landed); ad-hoc re-verification also works (72 cases is trivially enumerable).** |
...
| **Autokey on 73-char column mask extract** | **46,170 configs (PT/CT × Vig/Beau/VBeau × AZ/KA × 9 keep-col variants), max 6/24 = NOISE. Different CT and shifted crib positions vs raw 97 — genuinely independent test.** | **Requires column mask and shifted crib positions (E-COLMASK-AUTOKEY, 2026-03-12)** |
| **MITM: columnar trans × periodic sub on 73-char extract** | **31.8M configs (w7-9 exhaustive × p1-13 × 3 variants × 2 key modes + rail fence + route ciphers). Best 18/24 at w9/p11 = NOISE (Monte Carlo: ~3.6 expected hits by chance in full search).** | **Requires column mask and shifted crib positions (E-COLMASK-MITM-TRANS-SUB, 2026-03-12)** |
| **Grid-coordinate-derived keys on 73-char extract** | **3,726 configs (150+ key functions from row/col), max 5/24. Affine fit key=a*col+b*row+c (17,576 combos): no pattern ≥20/24. Required keystream is completely irregular.** | **Requires 28×31 grid layout (E-COLMASK-GRID-COORD, 2026-03-12)** |
| **Native grid reading orders + periodic sub on 73-char extract** | **2,262 configs (10+ reading orders × p1-26 × 3 variants). Best 22/24 at p24 = underdetermined noise. All discriminating-period scores (p≤7) ≤ ~12/24.** | **Requires 28×31 grid and column mask (E-COLMASK-NATIVE-GRID-READING, 2026-03-12)** |
