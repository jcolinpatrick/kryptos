...
| GAP-06 | Sanborn / Scheidt public-comment provenance | "Two systems", "masked English", "stego" all appear in public commentary. Project policy treats these as **Tier-3 community hearsay** per `feedback_sanborn_epistemic_weight.md` and `C-SANBORN-01`. Campaign 001 F2 (5 packs, 630 specs) was null. | Tier-3 hearsay does not specify what "two systems" means structurally — sub+trans, sub+sub, stego+cipher, two phases of one cipher, etc. Without a structural disambiguation the bridge cannot produce a non-arbitrary pack. | Either (a) a creator-attested mechanism statement with a specific structural commitment (e.g. "the second system is a route", with corroborating provenance), OR (b) a project policy decision to demote the claim to "no evidentiary weight" and stop authoring packs against it. | Two-layer or three-layer compositions; LESSON-022; LESSON-019. | None admissible — Tier-3 evidence cannot drive an admissible side-effect prediction without independent corroboration. | low | unowned (downgrade is the more likely outcome than corroboration) | open — likely permanent |
