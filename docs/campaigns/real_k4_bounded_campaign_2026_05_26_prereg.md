...
3. **Null-survived against the RIGHT null:** alert `p_value_status == ok_matched_family` AND `p_value_vs_null <= 1e-6`. **`ok_gated` does NOT count** — it means only the strawman `random_text` null was available (verified: that null's max crib_score is 7 over 100k samples, so any structured-but-wrong transposition output can clear it). `matched_null_miss` and `cache_miss` likewise disqualify. **The calibrated families that can produce `ok_matched_family` are `{vigenere, beaufort, variant_beaufort, columnar_single, columnar_double, rail_fence, myszkowski, route}`** (verified against `null_baselines/manifest.json`). The transposition trio `rail_fence / myszkowski / route` was calibrated 2026-05-26 specifically for this campaign (50k samples each; max crib_score 3/6/5 respectively — `rail_fence`'s family is so small (~23 distinct depths) that its "null" is a near-exhaustive enumeration, not a sample). A graded candidate in any *still-uncalibrated* family — `grille, polybius, quagmire`, or any multi-layer composition, which `alerts.py` routes to the random_text fallback — is recorded as **"uncalibrated — pending matched-null build,"** never as a non-artifact signal, until a matched-family null for that family is calibrated and the candidate re-evaluated. (This closes the artifact-masquerade path the red-team found: an unmatched-family artifact beating the strawman null and being stamped `ok_gated`.)
...
- [x] Red-team-disprover has reviewed THIS preregistration's design (per `feedback_red_team_before_swings`; campaign is real compute). — done 2026-05-26, blocker fixed, see §11.
- [ ] §4 locked definition of "non-artifact graded signal" reviewed and accepted by operator. — PENDING operator.
- [x] §9 readiness gate run green within 24h; commit hash recorded. — **GREEN at HEAD `b3658a1` (2026-05-26):** doctor 20/0; core `tests/` 2149 passed; `kryptosbot/tests/` 2481 passed / 1 skipped; self-test K1/15 K2/17 K3/9345, 0 false-positive breakthroughs; real-API K1 discovered (`$0.026`, 20/20); manifest re-pinned at HEAD.
- [ ] Launch command (§2) recorded in run log with timestamp. — at launch.
- [ ] Operator acknowledges R3 §2 open concerns (fallback reliability) and commits to checking §6.1.7 metrics at closure. — PENDING operator.
