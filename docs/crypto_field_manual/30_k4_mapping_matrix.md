...
| **Enigma / Hebern / rotor machines** | SUB-POLY-MECH | State-dependent (rotor stepping); non-periodic but deterministic from initial state; IC ~0.048-0.055 | [HYPOTHESIS] K5 shares coded words at same positions, suggesting position-dependent not state-dependent cipher. If this inference holds, eliminates all state-dependent ciphers. See `docs/kryptos_ground_truth.md` section C5 for caveats. | ELIMINATED-T1 [conditional on K5 hypothesis] | `docs/invariants.md` section 8 | CLOSED (conditional) |
| **Kryha / M-325 / KL-7** | SUB-POLY-MECH | Various mechanical polyalphabetic; state-dependent stepping | Same K5 position-dependency inference as Enigma [HYPOTHESIS] | ELIMINATED-T1 [conditional on K5 hypothesis] | `docs/invariants.md` section 8 | CLOSED (conditional) |
