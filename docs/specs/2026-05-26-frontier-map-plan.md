...
## As-built deviations (post-implementation, after two-stage + final review)

Landed across commits `58bbd5e..396f7b0`. Deviations from the spec/plan, all deliberate:

1. **Family universe = `KNOWN_FAMILIES`, not the spec's `valid_ledger_family_universe()` union.** Real-output review showed the union pulls in non-cipher junk (`admissibility`, `campaigns_final_checklist`, …) that made the rendered frontier meaningless. The grid now uses the 31 canonical cipher families from `kryptosbot.registries.KNOWN_FAMILIES`. Trade-off (accepted): a legitimate cipher family present only in historical-ledger strings and absent from `KNOWN_FAMILIES` would be invisible in the grid rather than shown as `open` — conservative (no false frontier signal). Fix forward if such a family is found: add it to `KNOWN_FAMILIES`.
2. **`render_open_frontier` is a per-non-direct-model summary, not a per-cell enumeration.** The original per-cell render advertised direct-carving "open" cells (e.g. `affine × direct_ct_pt`) that are eliminated project-wide but invisible to the ledger-only view — misleading. Render now surfaces only the four non-direct alignment models (genuinely unexplored everywhere) with breadth counts + example families. Consequence: the `limit` parameter caps model lines (≤4 in practice), so it is effectively never binding.
3. **`has_signal` status added** (commit `396f7b0`) so a signal-bearing cell is not mislabelled `explored_shallow`. `FrontierCell` carries `{family, alignment_model, status, n_tested, best_crib}`; the spec's `evidence_refs` field was not implemented (advisory map, rebuilt fresh each cycle — deferred).
4. **Critic soft signal scoped to the prompt block** (advisory steer). The post-verdict ledger/ranking annotation was descoped to keep the advisory invariant low-risk; the prompt block is the active mechanism.

**Operational note (not feature scope):** the null-baseline calibration freshness is keyed to git HEAD, so each commit in this feature re-staled the matched-null cache and tripped the alert-path tests until a rebuild. This is the K4-campaign postmortem's follow-up #2 (decouple cache freshness from HEAD, e.g. content-hash); it bit three times during this work and should be fixed before the next alert-gated run.
